...
class HTTPTransport(Transport):
    ...
    def __init__(
        self, base_url: str = "", headers: Dict[str, str] = None, timeout: float = 60.0
    ):
    ...
    def _handle_error(self, e: Exception, context: str = ""):
        ...
        if isinstance(e, httpx.HTTPStatusError):
            ...
            # Safely access text (httpx will error if not read and not
            # streaming closed properly)
            try:
                error_body = e.response.text
            except httpx.ResponseNotRead:
                # Force read if we catch it here? Or just fallback.
                # Usually if raise_for_status() triggers, we are in a state
