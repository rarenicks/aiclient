...
def test_openai_structured_output_payload():
    ...
    url, data = provider.prepare_request(
        "gpt-4o", messages, response_schema=schema, strict=True
    )
    ...
    # Schema key order checking might be tricky if dict not predictable,
    # but == checks contents usually.
...
def test_openai_structured_output_payload_no_strict():
    ...
    url, data = provider.prepare_request(
        "gpt-4o", messages, response_schema=schema, strict=False
    )
