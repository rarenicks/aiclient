...
async def test_agent_call_mcp_tool():
    ...
    with patch("aiclient.agent.MCPServerManager") as mock_manager_cls:
        ...
        async with agent:
            ...
            # Note: Checking history is harder with mocks, but model should be
            # called twice
...
async def test_agent_multiple_mcp_tools():
    ...
    with patch("aiclient.agent.MCPServerManager") as mock_manager_cls:
        ...
        async with agent:
            ...
            # If closure bug exists, it might call tool_b (last in loop) instead
            # of tool_a
